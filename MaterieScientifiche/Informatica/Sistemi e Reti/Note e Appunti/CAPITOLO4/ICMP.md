...
> > L’ICMP (Internet Control Message Protocol), RFC 792, fornisce un meccanismo di monitoraggio della rete, utilizzato prevalentemente dai router o dagli host destinatari per segnalare agli host mittenti eventuali insuccessi nelPinstradamento dei pacchetti.
...
| **11 Time Exceeded** | Sent when a packet has exceeded its maximum allowed time to travel across the network.  | A router might send this if a packet takes too long to reach its destination. |
...
| **3 Destination Unreachable** | Sent when a packet cannot reach its destination due to various reasons, such as a non-existent host or a network error. | A router might send this if it can't find the requested IP address. |
